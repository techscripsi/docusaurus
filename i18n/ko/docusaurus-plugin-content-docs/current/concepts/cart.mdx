...
<div class="container">
  <div class="row">
    <div class="col col--6">
      오른쪽 코드는 [장바구니 시작](/docs/api/init) 요청에 필요한 필수 속성만 포함된 간단한 요청의 예입니다.
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      OmniCart는 유효한 형식의 태그만 있으면 모든 태그를 허용합니다. 원하는 경우 각 [장바구니 시작](/docs/api/init) 요청에 대해 다른 태그 또는 태그 집합을 사용할 수 있습니다. 태그는 회원님 측에서 생성되므로 태그의 일관성은 회원님이 직접 관리합니다.

      오른쪽의 요청 스니펫에서 `mage` 및 `naga` 항목에는 `tower` 태그가 할당됩니다. `beholder` 항목에는 `dungeon` 태그가 표시되어 있고 `nomad` 항목에는 태그가 전혀 없습니다.
      
      카트가 시작된 후에는 태그를 사용하여 [장바구니 품목 상태 검색](/docs/api/status), <CancelLink />, [캡처](/docs/api/capture) 또는 <RefundLink /> 해당 API 메서드를 호출하여 품목을 확인할 수 있습니다(품목 상태에 대한 자세한 내용은 [결제 수명 주기](/docs/concepts/lifecycle) 참조).
...
* 태그를 정의하고 항목에 할당하지 않으면 정의된 태그가 요청 결과에 실질적인 영향을 미치지 않더라도 [장바구니 시작](/docs/api/init) 요청은 OmniCart에서 승인됩니다.
