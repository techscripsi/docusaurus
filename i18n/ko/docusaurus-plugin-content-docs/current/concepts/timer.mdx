...
타이머 상태는 다음을 사용하여 검색할 수 있습니다. [상태](/docs/api/status) 요청을 사용합니다. 이러한 상태는 항목별로 반환됩니다.
...
/>
    ...
    이 `started` 카운트다운이 켜져 있는 상태입니다. 남은 시간(초)은 의 값입니다. `remainingSecs` 의 속성 `timerSnapshot` 개체를 [status](/docs/api/status) 응답에 추가합니다.
