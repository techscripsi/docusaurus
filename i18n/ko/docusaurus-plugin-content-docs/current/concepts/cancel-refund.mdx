...
다음을 사용하여 전체 장바구니 또는 특정 장바구니 항목을 취소하거나 환불할 수 있습니다. <CancelLink /> 그리고 <RefundLink /> 각각 요청. 특정 장바구니 항목을 부분적으로 취소하거나 환불하는 옵션도 있습니다. 이를 위해 <ModifyLink /> request가 사용됩니다.
...
:::info
전체 장바구니를 취소하거나 환불한 후 모든 장바구니 항목이 `canceled` 또는 `refunded` status를 사용합니다.
:::
...
:::info
특정 카트 항목을 취소하거나 환불한 후 이러한 항목은 `canceled` 또는 `refunded` status를 사용합니다.
:::
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      다음을 사용할 수 있습니다. `tags` 그리고 `itemIds` objects와 `timerEnabled` 재산.
      :::warning 참고해주세요
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      이 `tags` criterion은 와의 중복을 무시합니다. `itemIds` 개체 속성을 사용합니다. 다음 예제를 참조하세요. 태그와 동일한 요청에서 지정된 태그로 표시된 항목을 지정합니다. OmniCart는 지정된 태그로 표시된 모든 항목을 취소 / 환불하고 항목이 실제로 태그와 식별자로 두 번 지정된다는 사실을 무시합니다.
...
:::warning 참고해주세요
부분 취소 또는 환불 후에도 항목은 `authorized` 또는 `completed` status를 사용합니다.
:::
