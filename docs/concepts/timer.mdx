...
Timer statuses can be retrieved using the [`status`](/docs/api/status) request. These statuses are returned per item.
...
/>
    ...
    The `started` status means that the countdown is on. The remaining time (in seconds) is the value of the `remainingSecs` property of the `timerSnapshot` object in the [status](/docs/api/status) response.
...
A timer enters the `elapsed` status when the number of seconds specified in the timer settings elapses. This is the end of a timer life cycle. From this status, the timer for the current item and `customerId` cannot be restarted. To start a new timer, initiate a new cart with the <InitLink /> request or enable a timer for an existing cart item with the <ModifyLink /> request.
