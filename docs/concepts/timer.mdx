...
/>
    ...
    The `started` status that the countdown is on. The remaining time (in seconds) is the value of the `remainingSecs` property of the `timerSnapshot` object in the [status](/docs/api/status) response.
