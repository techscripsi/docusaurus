...
<div class="container">
  <div class="row">
    <div class="col col--6">
      `paymentSnapshot` 객체는 <StatusLink /> 요청에 대한 응답으로 반환되는 항목 객체의 일부입니다. 여기에는 이전 <InitLink /> 또는 <ModifyLink /> 요청에서 정의된 값이 포함됩니다(이 페이지에서 [이전에](#totalamounts-itemamounts)에 제공된 설명 참조).
