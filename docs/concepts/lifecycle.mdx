...
<div class="container">
  <div class="row">
    ...
    The `expired` status means that the time set in the `cartExpiry` property passed with the <InitLink /> request has elapsed. The possible reasons are that the customer decided not to proceed with the payment or was unable to do it.
...
A payment enters the `canceled` state only after it has been fully canceled with the <CancelLink /> request. A payment can also be auto canceled if the OmniCart settings and your privileges allow that.
...
The `ongoing` status means that a payment is in an intermediate state. Some <ModifyLink /> requests as well as partial cancellations or refunds might take longer to process than other requests. This status is also returned after the <InitLink /> request when customer action is required.
