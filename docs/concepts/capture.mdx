...
You can capture the entire cart or specific items. There is also an option to use the [`modify`](/docs/api/40-modify) request to reduce an item amount before capturing it (see the [Modifying Items](/docs/concepts/modify) section for more information). 
