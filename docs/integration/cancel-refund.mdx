...
Cancellation and refund are two different operations that can be done on different stages of the [payment life cycle](/docs/integration/payment-lifecycle).
...
Use the [`status`](/docs/api/15-status) request to check the current item statuses.
:::
...
You can cancel or refund an entire cart or specific cart items with the [cancel](/docs/api/20-cancel) and [refund](/docs/api/50-refund) requests respectively. There is also an option to partially cancel or refund specific cart items. To do that, the [`modify`](/docs/api/40-modify) request is used.

:::warning Please, note
Neither the [`cancel`](/docs/api/20-cancel) nor [`refund`](/docs/api/50-refund) request affects item timers. If you need to change timer settings, use the [`modify`](/docs/api/40-modify) request. See the [Item Timers](/docs/integration/timer) for more information.
:::

## Entire Cart {#entire-cart}
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      To cancel or refund an entire cart, use the [cancel](/docs/api/20-cancel) or [refund](/docs/api/50-refund) request respectively. The only mandatory property is `cartId` passed with the [init](/docs/api/10-init) request earlier.
    </div>
    <div class="col col--6">
      ```js
      {
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      To cancel or refund specific cart items, use the [cancel](/docs/api/20-cancel) or [refund](/docs/api/50-refund) request respectively. The specify the request scope, apart from the mandatory `cartId` value, provide the criteria for the items to be canceled or refunded in the `filterBox` object. 
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      If you pass the `timerEnabled` property, the request scope will be narrowed down to the items based on their timer settings (see the [Item Timers](/docs/integration/timer) section for more information). 
...
With OmniCart, it is possible to partially cancel or refund cart items. For example, this might be needed if a customer paid for a product and, for some reason, the product cannot be (or was not) delivered in full quantity. To do this, use the [`modify`](/docs/api/40-modify) request to change the item amount to a lower value. Note that you cannot increase the item amount with the `modify` request (see the [Modifying Items](/docs/integration/modify) and the [Amounts](/docs/integration/amounts) sections for more information). The difference between the initial amount and the modified amount will be returned to the customer automatically.

Partial cancellation or refund can be done multiple times. That said, to fully cancel or refund an item, use the [`cancel`](/docs/api/20-cancel) and [`refund`](/docs/api/50-refund) requests respectively, as changing an item amount to zero is not allowed.
