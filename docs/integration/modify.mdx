...
The [`modify`](/docs/api/40-modify) request allows you to modify a previously initiated cart. This request is similar to the [`init`](/docs/api/10-init) request in that it allows you to define properties on the cart, tag, and item levels (see the [Carts, Tags, and Items](/docs/integration/cart) section for more information). Although, there are a number of differences that are covered in this section.

The [`modify`](/docs/api/40-modify) request allows you to:
...
:::warning Please note
With the [`modify`](/docs/api/40-modify) request you cannot change the `cartId` value or the values of the cart item identifiers.
:::

<!--FIXME can be changed separately or in combinations -->

Some of the cart properties can only be modified when items are in a particular payment status (see the [`Payment Life Cycle`](/docs/integration/payment-lifecycle) section for more information on payment statuses), others can be modified regardless of any payment status.

:::note
The [`modify`](/docs/api/40-modify) request request does not change the item status.
:::
...
Tags can be modified regardless of the payment status, unless it involves changing amounts (see [Modifying Payment Settings](#payment-settings) below for details).
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      Consider the example to the right. The cart was initiated with the `tower` tag that was applied to the `genie` and `naga` items. Then the cart was modified and the `inferno` tag was introduced. The new tag was assigned only to the `genie` item so the `genie` full item amount is now recalculated by OmniCart according to the `inferno` tag settings. It is now multiplied by the `amountModifier` value since `amountMode` is set to `calculated` (see the [Amounts](/docs/integration/amounts) section for more information on amounts). 
      
      The `naga` item is still marked with the initial `tower` tag. However, this tag was not redefined in the [`modify`](/docs/api/40-modify) request. Since there were no new settings for the `tower` tag, the `naga` item kept its previous amount settings. In fact, any other undefined tag assigned to the `naga` item would have produced the same effect. That said, the [`status`](/docs/api/15-status) request returns the latest tag assigned to an item regardless of whether its settings were applied to that item or not. So here's a tip.
      :::tip
      Manage your tags.
      :::
    </div>
...
Amount settings ​​can be modified on the cart, tag, or item level as with the [`init`](/docs/api/10-init) request (see also the [Cart, Tags, and Items](/docs/integration/cart) section for more information). An item being modified must be in the `authorized` or `completed status` (see the [Payment Life Cycle](/docs/integration/payment-lifecycle) section for more information). 
...
There is no limit on the number of times the [`modify`](/docs/api/40-modify) request can be used to reduce the amount. However, to fully cancel or refund an item, use the [`cancel`](/docs/api/20-cancel) and [`refund`](/docs/api/50-refund) methods respectively, since OmniCart does not accept zero amounts.

:::info
See the [Cancellation and Refund](/docs/integration/cancel-refund#specific-items-partial) section for more information.
:::
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      The [`modify`](/docs/api/40-modify) request introduced the following changes:
      ...
      * `"quantity": 4` was added.
      ...
      :::info
      See the [Amounts](/docs/integration/amounts) section for more information.
      :::
    </div>
    <div class="col col--6">
      ```js title="Legend"
  ...
  <div class="row">
    <div class="col col--6">
      ...
      * `amount` was changed from `1500` to `3000`.
      ...
      :::info
      See the [Amounts](/docs/integration/amounts) and [Cancellation and Refund](/docs/integration/cancel-refund#specific-items-partial) sections for more information.
      :::
    </div>
    <div class="col col--6">
      ```js title="Legend"
...
With the [`modify`](/docs/api/40-modify) request request you can:
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      ...
      The `modify` request changed the trigger event to `initiated`. Since the `initiated` status precedes the `captured` status, the timer switches to the `started` status and starts countdown after the [`modify`](/docs/api/40-modify) request has been processed.
    </div>
    <div class="col col--6">
      <Tabs>
        <TabItem value="status before">
