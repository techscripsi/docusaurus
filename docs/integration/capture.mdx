...
You can capture the entire cart or specific items. There is also an option to use the [`modify`](/docs/api/40-modify) request to reduce an item amount before capturing it (see the [Modifying Items](/docs/integration/modify) section for more information). 
...
<div class="container">
  <div class="row">
    <div class="col col--6">
      To capture an entire cart, use the [capture](/docs/api/30-capture) request respectively. The only mandatory property is `cartId` passed with the [init](/docs/api/10-init) request earlier.
    </div>
    <div class="col col--6">
      ```js
      {
...
With OmniCart, it is possible to reduce an item amount before capturing. For example, this might be needed if a customer paid for a product and, for some reason, the product cannot be (or was not) delivered in full quantity. To do this, use the [`modify`](/docs/api/40-modify) request to change the item amount to a lower value. Note that you cannot increase the item amount with the `modify` request (see the [Modifying Items](/docs/integration/modify) and the [Amounts](/docs/integration/amounts) sections for more information). The difference between the initial amount and the modified amount will be canceled. After that, you can use the [`capture`](/docs/api/30-capture) request to capture the remaining amount.