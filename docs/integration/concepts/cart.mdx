...
<Columns>
  <Column>
    ...
    The `pegasus` object here is an item identifier that contains the mandatory `amount` property (see [Amounts](/docs/integration/concepts/amounts) section for more information on handling amounts).
...
<Columns>
  <Column>
    ...
    Tags can also be redefined or reassigned by calling the [modify](/docs/api/40-modify) method — this option is described in detail in the [Modify](/docs/integration/concepts/modify) section of this documentation.
  </Column>
  <Column>
    ```js
        "cart": [
